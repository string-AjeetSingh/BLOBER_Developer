...
exports.rough = (req, res) => {
    ...
    if(req.body.setUser){
        ...
        result = 'attenpt to sett the user';
    }
    ...
    */
    ...
    res.status(200);
    res.json({ "Hello": "Hero", "body": req.body, "result": result });
...
async function logit(param) {     //working fine
    ...
    if (result.length < 1) {           //check if userName not exists
        ...
        state = curr.insertOne({ email: param.email, pass: param.pass, prevNo: 0, posts: [], doc: 'user' }); ``
